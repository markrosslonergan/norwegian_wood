...
int main(int argc, char* argv[])
{
	...
	for(auto &mode: modes){
		...
		for(double dcp = 0; dcp < 360; dcp +=1.0){

			phases.at(0) = dcp;
			SBNprob myprob(4, angles, phases, mass);
			
			SBNprob myantiprob(4, angles, phases, mass);
			myantiprob.useAntiNeutrino = true;
			myantiprob.init();

			if(mode == "3p0vac" || mode == "3p1vac")
			{
				myprob.setMatterEffect(false);
				myantiprob.setMatterEffect(false);
			}else{
				myprob.setMatterEffect(true);
				myantiprob.setMatterEffect(true);
			}
			if(mode == "3p1mat_nonc"){
				myprob.setNCMatterEffect(false);
				myantiprob.setNCMatterEffect(false);
			}

			dunestream<<dcp<<" "<<myprob.probabilityMatterExact(0,0,2.0,1300)<<" "<<myprob.probabilityVacuumExact(0,0,2.0,1300)<<" "<<myantiprob.probabilityMatterExact(0,0,2.0,1300)<<" "<<myantiprob.probabilityVacuumExact(0,0,2.0,1300)<<std::endl;
		}
